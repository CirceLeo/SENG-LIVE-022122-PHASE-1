...
function renderPokemon(char) {
  ...
  pokeCard.addEventListener("click", () => showCharacter(event, char));
...
function showCharacter(e, char){
  //if ((e.target.className =! "likeBttn") || (e.target.className =! "deleteBttn")){
  fetch(`http://localhost:3000/characters/${char.id}`)
  .then(info => info.json())
  .then(charObj => {
    const soloChar = renderPokemon(charObj)
    soloChar.id = 'poke-show-card'
    //console.log(soloChar)
    pokeContainer.replaceChildren(soloChar)
  })
  //}
}

function getPokemon(){
  const pokeLink = "http://localhost:3000/characters"
  fetch(pokeLink)
  .then(recieved => recieved.json())
  .then(pokeObj => pokeObj.forEach(renderPokemon))
}
